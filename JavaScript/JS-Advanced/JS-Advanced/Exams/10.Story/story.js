class Story {
    ...
    comment(username, content, id) {
        ...
        let commentReplyTo = this._comments.find(c => c.Id === id);
        let replyNextId = commentReplyTo.Replies.length + 1;
        let replyId = Number(`${commentReplyTo.Id}.${replyNextId}`);
        let reply = {
            Id: replyId,
            Username: username,
            Content: content,
        ...
        commentReplyTo.Replies.push(reply);
    ...
    toString(sortingType) {
        ...
        for (const cmt of comments) {
            let commentString = `-- ${cmt.Id}. ${cmt.Username}: ${cmt.Content}`;
            let repliesString = cmt.Replies
                .map(r => `--- ${r.Id}. ${r.Username}: ${r.Content}`)
                .join(`\n`);

            repliesString = cmt.Replies.length > 0
                ? `\n${repliesString}`
                : '';

            let combinedString = `${commentString}${repliesString}`;
            commentsResult.push(combinedString);
        }

        let finalComments = this._comments.length > 0
            ? `\n${commentsResult.join('\n')}`
            : '';
        let result = [];
