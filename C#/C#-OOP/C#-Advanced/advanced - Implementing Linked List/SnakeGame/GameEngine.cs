...
namespace SnakeGame
{
    public class GameEngine
    {
        ...
        public void Start()
        {
            ...
            {
                //var newHeadPosition = Snake.SnakeBody.Head.Value.GetNewPosition(movement);
                BoundariesChecker.CheckBoundaries(Snake.SnakeBody.Head.Value, movement);
                Snake.Move(movement);
                if(Snake.CheckSelfCanibalism())
                {
                    Console.Clear();
                    ConsoleHelper.Write(new Position(0, 0), "Game over!");
                    Console.WriteLine();
                    Console.Write($"Your final score: {Score.Points}"); 
